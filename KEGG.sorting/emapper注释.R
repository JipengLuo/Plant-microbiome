...
head(gene2go)
write.csv(gene2go, file = "GO_genes.csv")
...
********************************************************************************
##导入KEGG注释参考表格
kegg <- read.csv("kegg.pathway.table.csv", header = T, row.names = 1)

###根据kegg和gene2ko两个表格共有的'Ko'元素保留共有的基因
ddd <- gene2ko %>% 
  left_join(kegg, by = "Ko") %>% 
  dplyr::select(GID,level1,level2,level3,pathway_id,Ko,gene,KO_name, EC) %>% 
  na.omit()
###保留唯一的Ko
ddd.unique <- unique(ddd)

##导入磷循环丰度数据
ppp <- read.csv("P.gene.abundance.csv", header = T, row.names = 1)

library(dplyr)
# 将ppp的行名转换为一个普通列，这里假设新列名为'rowname'
ppp$GID <- rownames(ppp)

##保留P循环基因所有GID,然后选择KEGG中元素；ppp应该在左侧，保留其所有观察；
fff <- ddd %>% 
  left_join(ppp, by = "GID") %>% 
  dplyr::select(GID,s01,s02,s03,s04,s05,s06,s07,s08,s09,s10,s11,s12,s13,s14,s15) %>% 
  na.omit()

dim(ddd)
dim(ppp)
# 去掉重复的行;方法一，这行代码将返回一个新的数据框fff_unique，其中包含了fff中所有唯一的行
fff.unique <- unique(fff)

#方法二，采用dplyr包中的distinct()函数
library(dplyr)
df_unique <- fff %>% distinct()

fff2 <- ddd %>% 
  left_join(fff.unique, by = "GID") %>% 
  dplyr::select(GID,s01,s02,s03,s04,s05,s06,s07,s08,s09,s10,s11,s12,s13,s14,s15,
                level1,level2,level3, pathway_id, Ko,gene, KO_name, EC) %>% 
  na.omit()

write.csv(fff2, file = "Pgene.kegg.csv")
***********************************************************************************
...
makeOrgPackage(gene_info=gene_info,
               ...
               maintainer = "luojip <luojip@iu.edu>",  #修改为你的名字和邮箱
               author = "luojip <luojip@iu.edu>",  #修改为你的名字和邮箱
               outputDir = "./",  #输出文件位置
...
# KEGG pathway 富集；要先计算表达上调和下调的基因
